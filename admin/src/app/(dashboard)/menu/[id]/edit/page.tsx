...
export default function EditMenuItem() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.put(`/menu/${params.id}`, {
        ...formData,
        price: parseFloat(formData.price)
      });
      toast.success('Veg item updated successfully');
      router.push('/menu');
    } catch (error: any) {
      toast.error(error.response?.data?.details?.[0] || 'Failed to update item');
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100">
          ...
          <button type="submit" disabled={loading} className="px-4 py-2 bg-primary hover:bg-green-700 text-white rounded-md text-sm font-medium shadow-sm disabled:opacity-50">
            {loading ? 'Saving...' : 'Update Veg Item'}
          </button>
        </div>
      </form>
    </div>
